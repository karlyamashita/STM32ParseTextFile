...
/*
 * Description: Must call SendCanTxMessage1() in polling routine in order for any messages in buffer to be sent.
 *
 * Input: CAN handle
 * Output: The number of CAN messages left in buffer
 *
 */
...
/*
 * Description: Add CAN message to Tx buffer. Must call SendCanTxMessage1() in polling routine.
 *
 * Input: CAN message
 * Output: none
 *
...
/*
 * Description: Add received CAN message to Rx buffer. Must call Can1DataAvailable()
 *
 * Input: CAN message
 * Output: none
 *
...
void AddCanRxBuffer1(CanRxMsgTypeDef *canMsg) {
	...
	DRV_RingBuffPtr__Input(&RxMessagePtr1, CAN_MAX_RX_BUFF); // increment input buffer ptr
}

/*
 * Copy CAN buffer to canMsg array
...
void AddCanRxBuffer2(CanRxMsgTypeDef *canMsg) {
	...
	DRV_RingBuffPtr__Input(&RxMessagePtr2, CAN_MAX_RX_BUFF); // increment input buffer ptr
}
