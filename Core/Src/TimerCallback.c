...
/*
function: Register a callback function with shutdown delay. Iterate through TimerCallbackArray until a free spot is found in array. Copy callback and timerCount to array. ShutDownEnable is false by default.
...
output: the timer array pointer. 0 if no array available, -1 if defined already, -2 if null callback
*/
int8_t TimerCallbackWithShutDownRegister(TimerCallback callback, uint32_t timerValue, bool repeat, uint32_t timerShutDownValue) {
	int i = 0;
	if(callback == 0) return -2; // null callback
	while(timerCallback[i].callback != 0) {
		if(timerCallback[i].callback == callback) {
